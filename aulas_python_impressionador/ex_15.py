...
cadastro = input('Cadastre o produto desejado: ').lower()
print(cadastro)
if cadastro in produtos:
    print('Produto já possui cadastro! Tente novamente!!')
else:
    produtos.append(cadastro)
    print(f'Produto {cadastro}, cadastrado com sucesso.')
    print(produtos)'''
...
produto_procurado = input('Informe o produto que você deseja: ').lower()
if produto_procurado in produtos:
    indice = produtos.index(produto_procurado)
    precos = precos[indice]
    print(f'O produto {produto_procurado}, custa {precos} reais.')
else:
    print(f'O produto {produto_procurado}, não existe na lista!')

    cad_novo_produto = input(
        'Deseja cadastrar um novo produto? Digite agora: s/n:  ').lower()
    if 's' in cad_novo_produto:
        cad_novo_produto = produtos.append(produto_procurado)
        print(cad_novo_produto)
        print(produtos)
    else:
        print('FIM')
    # n = ('Fim')
...
vendas = [
    [10, 20, 100, 80, 90, 100, 20, 30, 44, 55, 33, 34, 100, 90,
        80, 39, 87, 45, 50, 50, 50, 50, 40, 30, 3, 93, 39, 49, 88],
    [100, 1, 1, 4, 5, 90, 100, 20, 4, 5, 100, 100, 100, 100, 100,
        93, 20, 15, 40, 90, 90, 90, 90, 90, 90, 33, 22, 44, 43, 34],
]
